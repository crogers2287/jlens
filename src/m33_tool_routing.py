#!/usr/bin/env python3
"""M33 telemetry-gated tool routing on the frozen M30 trigger."""
...
POLICIES = ("no_repair", "tool_on_every_task", "random_trigger_tool",
            "telemetry_trigger_tool")
CAPTURE_DIR = "m33_qwen15_moe"


def load_manifest(path):
    manifest = json.loads(Path(path).read_text())
    if manifest.get("selection_status") != \
            "predeclared_before_m33_task_generation":
        raise ValueError("M33 manifest is not predeclared before generation")
    bands = manifest.get("bands") or []
    tasks = manifest.get("tasks") or []
    if manifest.get("n_tasks") != 384 or len(tasks) != 384 or len(bands) != 6:
        raise ValueError("M33 manifest must declare six bands and 384 tasks")
    if len({task["task_id"] for task in tasks}) != 384:
        raise ValueError("M33 task ids must be unique")
    counts = Counter(task["band"] for task in tasks)
    for band in bands:
        if band.get("n_tasks") != 64 or counts[band["band_id"]] != 64:
            raise ValueError(f"M33 band size mismatch: {band['band_id']}")
    if manifest["frozen_trigger"].get("threshold_fail_probability") != 0.5:
        raise ValueError("M33 frozen trigger threshold changed")
    if tuple(sorted(manifest["policies"])) != tuple(sorted(POLICIES)):
        raise ValueError("M33 policy set changed")
    return manifest


def prior_tuples_through_m32(m29_path, m30_path, m31_path, m32_path):
    """Every operand tuple generated by the M29-M32 manifests."""
...
def generate_tasks(manifest, m29_path, m30_path, m31_path, m32_path):
    ...
    prior = prior_tuples_through_m32(m29_path, m30_path, m31_path, m32_path)
    ...
    if len(tasks) != 384 or len({t["prompt_id"] for t in tasks}) != 384:
        raise ValueError("M33 generation did not produce 384 unique tasks")
...
def tool_answer(task):
    ...
    a, op, b = task["a"], "*", task["b"]
    assert task["expression"] == f"{a}{op}{b}"
    return str(a * b)


def run_captures(manifest, tasks, model_ref, captures_root):
    """Single-model-load greedy originals capture (only M33 GPU phase)."""
    ...
    print(f"[jlens] M33 originals capture ({len(tasks)})", flush=True)
    for task in tasks:
    ...
    print("[jlens] M33 captures complete", flush=True)
...
def evaluate(manifest, rows):
    ...
    rng = random.Random("m33:random-trigger")
    ...
    for policy, attempted in attempted_by_policy.items():
        ...
        for row in rows:
            ...
            if row["task_id"] in attempted:
                invocations += 1
                if final_pass:
                    false_alarms += 1  # verifier-first: original kept
                elif row["tool_pass"]:
                    final_pass = True
                    rescued += 1
        ...
        if any(o < s for o, s in zip(outcomes, original_success)):
            raise ValueError("M33 stop: a policy replaced a passing original")
    ...
    gate = manifest["detector_reproduction_gate"]
    gate_pass = (precision is not None and recall is not None
                 and precision >= gate["min_precision"]
    ...
    h2_verdict = ("efficient"
                  if gate_pass and retention is not None
                  and retention >= 0.80 and invocation_fraction <= 0.60
                  else "not_established")
    if not gate_pass:
        h1_verdict = "gate_failed_no_claim"
        h2_verdict = "gate_failed_no_claim"
    ...
    return {
        "schema_version": 1,
        "run_kind": "m33_tool_routing_evaluation",
        ...
        "frozen_trigger": {
            ...
            "reproduction_gate": {**gate, "passed": gate_pass},
        },
...
def main(argv=None):
    ...
    ap.add_argument("--tasks-out",
                    default="reports/shadow/private/m33_hf_prompts_local.jsonl")
    ...
    ap.add_argument("--telemetry-out",
                    default="reports/shadow/private/"
                            "m33_hf_telemetry_records_local.jsonl")
    ap.add_argument("--rows-out",
                    default="reports/shadow/private/m33_routing_rows_local.jsonl")
    ap.add_argument("--summary-out",
                    default="reports/telemetry/hf_m33_routing_run_summary.json")
    ap.add_argument("--evaluation-out",
                    default="reports/telemetry/hf_m33_routing_evaluation.json")
    args = ap.parse_args(argv)

    manifest, tasks = prepare_private_tasks(
        args.manifest, args.m29_manifest, args.m30_manifest,
        args.m31_manifest, args.m32_manifest, args.tasks_out)
    root = Path(args.captures_root)
    if args.prepare_only:
        done = sum((root / CAPTURE_DIR / f"{t['prompt_id']}.pt").exists()
                   for t in tasks)
        print(f"[jlens] M33 predeclared tasks: {len(tasks)}; "
    ...
    adapter = EXEC.FixtureRetrievalAdapter({
        "*": {"source_kind": "public_fixture",
              "context": "M33 controlled local fixture context",
              "confidence": 0.8}})
    ...
    for task in tasks:
        ...
        if not path.exists():
            raise FileNotFoundError(f"missing M33 capture: {task['prompt_id']}")
        ...
        if action["action_type"] != "checker_needed":
            raise ValueError(
                f"M33 action drift: {task['prompt_id']} -> "
        ...
        if original_verdict == "undecided":
            ...
            continue
        label_row = {"task_id": task["prompt_id"], "band": task["m33_band"]}
        ...
        rows.append({
            "task_id": task["prompt_id"], "band": task["m33_band"],
            "p_fail": p_fail,
    ...
    summary = {
        "schema_version": 1,
        "run_kind": "m33_tool_routing_dataset",
        "task_category": manifest["task_category"],
        "n_tasks": len(tasks),
        ...
        "operands_disjoint_from_m29_m30_m31_m32": True,
        ...
        "candidate_only": True,
        "production_gated": True,
        "privacy_check_status": "aggregate-only tool routing summary",
    }
    Path(args.summary_out).write_text(json.dumps(summary, indent=1) + "\n")
    trig = evaluation["frozen_trigger"]
    h1 = evaluation["h1_routing_usefulness"]["verdict"]
    h2 = evaluation["h2_routing_efficiency"]["verdict"]
    print(f"[jlens] M33: {len(rows)} rows; gate_pass="
          f"{trig['reproduction_gate']['passed']} "
          f"(precision={trig['trigger_precision']} "
          f"recall={trig['trigger_recall']}); H1={h1}; H2={h2}", flush=True)
    return 0
